"""
Builds the two profile JSONs (financial-behavior + psychometric/behavioral)
from real player data. Every field is either:

  (a) a direct aggregate of something the game actually captures, or
  (b) an explicitly-labeled HEURISTIC PROXY built from what's available
      (documented inline, at the point it's computed), or
  (c) None, for anything the current game has no mechanic to generate at
      all (no investment/portfolio feature, no loan/credit feature, no
      validated psychometric instrument, no framing-experiment design for
      loss aversion, etc.) -- see NOT_AVAILABLE_FIELDS below.

Nothing in category (b) should be presented as a validated psychometric
score without a real methodology review -- these are reasonable-effort
proxies built to unblock the admin dashboard, not peer-reviewed measures.

DECISION VALUE CLASSIFICATION
------------------------------
The 5 advisory scripts (arjun/riya/meera/vikram/aahan) each use their own
option value names (see frontend/src/data/advisoryScripts.js), but they
map onto the same underlying 3-way pattern: spend-it-all / middle-ground /
save-or-protect-it. This map is what lets money_management_style and
time_preference be computed at all across different NPCs.
"""
...
DISCIPLINE_SCORE = {
    # impulsive / high-risk / spend-it-all -> 0.0
    "spend": 0.0, "trip": 0.0, "upgrade": 0.0, "reckless": 0.0,
    # middle-ground / hybrid -> 0.5
    "compromise": 0.5, "balanced": 0.5,
    # disciplined / protective -> 1.0
    "save": 1.0, "stiff": 1.0,
}

CATEGORY_KEYS = ["budgeting", "savings", "credit_debt", "investment", "insurance", "tax"]

# Fields from the two schemas that the current game has NO mechanic to
# generate -- no portfolio/investment feature, no loan-taking feature, no
# validated personality instrument, no framing-manipulation experiment.
# Listed here (rather than silently omitted) so the dashboard team can
# tell "not measured yet" apart from "measured as zero."
...
def build_financial_profile(player: models.Player, db: Session) -> dict:
    quiz_attempts = db.query(models.QuizAttempt).filter(models.QuizAttempt.player_id == player.id).all()
    advisory = db.query(models.AdvisoryChoice).filter(models.AdvisoryChoice.player_id == player.id).all()
    completions = player.quest_completions

    # --- financial_literacy: real accuracy per QUEST_CATEGORY_MAP category ---
    ...
    # HEURISTIC PROXY: if wrong answers are, on average, answered FASTER
    # than correct ones, that's a rough overconfidence signal (answering
    # fast without actually knowing). Not a validated construct.
    ...
    # --- money_management_style: from the 5 NPC dilemma choices ---
    # Pairs kept together deliberately -- filtering choice_value and score
    # into two SEPARATE lists and zipping them back together later would
    # silently misalign the moment any choice_value falls outside
    # DISCIPLINE_SCORE (e.g. a future NPC script not yet added to the map).
    ...
    style = {
        ...
        "avg_days_to_spend_windfall": None,  # not available -- see NOT_AVAILABLE_FIELDS
    }
    ...
    risk_and_investment = {
        # Aahan's dilemma (crypto vs index fund vs liquid debt fund) is the
        # only genuine risk-preference signal the game has -- everything
        # else in this block needs a real portfolio feature to measure.
...
def build_behavioral_profile(player: models.Player, db: Session) -> dict:
    advisory = db.query(models.AdvisoryChoice).filter(models.AdvisoryChoice.player_id == player.id).all()
    quiz_attempts = db.query(models.QuizAttempt).filter(models.QuizAttempt.player_id == player.id).all()
    ...
    # HEURISTIC PROXY: normalizes avg decision time into a 0-100
    # "deliberation speed" score against a 30s reference ceiling. Arbitrary
    # reference point, not a validated timing model.
    ...
    # Reuses the same discipline signal as money_management_style -- the
    # advisory dilemmas ARE literally spend-now-vs-save-for-later choices,
    # so this is a legitimate (if simple) proxy for delayed gratification.
    # hyperbolic_discounting_tendency genuinely needs a delay/amount
    # trade-off design the game doesn't have -- left None.
    ...
    # robotHelpLine text in every advisory script nudges toward the
    # disciplined/balanced option -- so "did they end up on that option
    # after asking for help" is a reasonable advice-susceptibility proxy.
